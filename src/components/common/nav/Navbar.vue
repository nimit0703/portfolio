<template>
    <div class="sticky-top" :class="`bg-${currentTheme}`">
      <nav class="navbar border-bottom border-1">
        <span class="poppins-regular fs-3 mt-sm-4">
          Nimit Trevadiya
        </span>
        <div class="d-flex mt-sm-4">
          ...
          <div class="d-flex align-items-center justify-content-between">
            ...
            <v-switch v-model="selectedTheme" hide-details inset class="mr-3" @change="handleThemeChange"></v-switch>
          </div>
        </div>
      </nav>
    </div>
  ...
  const isSmallScreen = computed(() => window.innerWidth <= 767);
  </script>
